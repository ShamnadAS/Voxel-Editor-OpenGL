...
Vector3 Matrix3::getAngle() const
{
    ...
    if(m[8] < 0) //m[8] = Cos(angleX) * Cos(angleY) 
    {
...
//Projection matrices
Matrix4 Matrix4::perspective(float l, float r, float t, float b, float f, float n) 
{
    Matrix4 matrix;
    matrix[0] = 2 * n / (r - l);
    matrix[5] = 2 * n / (t - b);
...
Matrix4 Matrix4::perspective(float fov, float aspectRatio, float f, float n) 
{
    float tangent = tanf(fov * DEG2RAD / 2);
    float t = tangent * n;
